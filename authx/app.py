...
<head>
  ...
  <style>
    :root {{ --ink: #20242a; --muted: #626b77; --line: #d9dee7; --surface: #fff; --band: #f4f7fb; --accent: #0f766e; --danger: #b42318; font-family: Arial, Helvetica, sans-serif; }}
    * {{ box-sizing: border-box; }}
    ...
    .badge {{ border-radius: 999px; padding: 4px 9px; color: #fff; font-size: 12px; font-weight: 700; background: var(--danger); }}
    ...
    .ok {{ border-left-color: #146c43; background: #eef7f1; color: #14532d; }}
    .muted {{ color: var(--muted); }}
    @media (max-width: 680px) {{ header {{ align-items: flex-start; }} .nav-actions {{ width: 100%; }} button, .button {{ width: 100%; text-align: center; }} }}
  </style>
</head>
<body>
  <header><div class="brand"><h1>AuthX</h1><span class="badge">VULNERABLE</span></div>{nav}</header>
  <main>{body}</main>
</body>
</html>"""
...
def create_app(db_path: Path) -> Flask:
    ...
    def register() -> Response | str:
        ...
        if not email or not password:
            return page_register(notice("Email and password are required.", "error"))
        try:
            cursor = db.execute(
            ...
            return page_register(notice("Email already registered.", "error"))
    ...
    def login() -> Response | str:
        ...
        if user is None:
            return page_login(notice("User not found.", "error"))
        if password != user["password_hash"]:
            return page_login(notice("Wrong password.", "error"))
    ...
    def forgot() -> str:
        ...
        user = db.execute("SELECT * FROM users WHERE email = ?", (email,)).fetchone()
        if user is None:
            return page_forgot(notice("No account for this email.", "error"))
        ...
        return page_forgot(link_notice("Lab reset link:", href, href))
    ...
    def reset() -> str:
        ...
        if user is None:
            return page_reset(token, notice("Invalid reset token.", "error"))
    ...
    def dashboard() -> Response | str:
        ...
        <section class="panel"><h3>Mode behavior</h3><p>This version is intentionally vulnerable.</p></section>""")
